...
base=$(basename "$aln" .prank.best.fasta.gz)
...
gzip -cd "$aln" | awk '
# Read FASTA file, remove whitespace and line breaks
BEGIN { RS=">"; ORS="" }
NR > 1 {
    n = split($0, a, "\n")
    seq = ""
...
END {
    # Confirms there are two sequences and they are the same length (they are properly aligned)
    if (count != 2) {
