######
# Goal
#   Plot dS vs. chrZ position (and chrW position) for gametolog genes.
# Contents
...
# Read in gametolog coords table
coords <- read_tsv("heloderma_Z_W_gametologs_final.tsv", show_col_types = FALSE) %>% 
  select(-dS)

# === 1) Make table for plotting ===
# Read in dN, dS, omega summary table
prank <- read_tsv("paml/paml_yn00_summary.tsv", show_col_types = FALSE)
...
# Separate pair name and add gene coords
plot_tbl_prank <- prank %>% 
  separate(pair, into = c("W", "Z"), sep = "__", remove = FALSE) %>% 
  left_join(coords, by = c("W", "Z")) %>% 
...
plot_prank_w <- ggplot(plot_tbl_prank, aes(x = W_start, y = dS)) +
  geom_point() +
  coord_cartesian(ylim = c(0, 0.6), xlim = c(0,70)) +
  theme_classic() +
  theme(legend.position = "none") +
