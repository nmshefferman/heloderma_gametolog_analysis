...
w_to_all <- read_tsv("blast_results/wGenes_autosomes.tsv", comment = "#", col_names = FALSE)
...
colnames(w_to_all)[1:12] <- c("W", "Autosomes", "Percent identity", "Alignment length", "Number mismatch", "Number gap openings", "Start alignment query", "End alignment query", "Start alignment subject", "End alignment subject", "Evalue", "Bit score")
...
w_to_all <- w_to_all %>%
  mutate(W = str_remove(W, "-mRNA-.*"),
         Autosomes = str_remove(Autosomes, "-mRNA-.*"))
...
autosome_coords <- read_tsv("gene_coords/autosome_gametolog_coords.gff3", comment = "#", col_names = c("seqid", "source", "type", "start", "end", "score", "strand", "phase", "attributes"))
...
autosome_coords <- autosome_coords %>% 
...
# Best autosome hit for each W gene
w_auto_summary <- w_to_all %>%
  group_by(W) %>%
  slice_min(order_by = Evalue, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  select(W, autosome_hit = Autosomes, autosome_evalue = Evalue, autosome_length = `Alignment length`)

# Final table
w_z_autosomes_summary <- w_z_summary %>%
  left_join(w_auto_summary, by = "W") %>% 
  left_join(w_coords, by = c("W" = "W_gene")) %>% 
  left_join(z_coords, by = c("Z_hit_1" = "Z_gene")) %>% 
  left_join(z_coords, by = c("Z_hit_2" = "Z_gene")) %>% 
  left_join(autosome_coords, by = c("autosome_hit" = "autosome_gene")) %>% 
  rename(
    Z_start_1 = Z_start.x,
    Z_end_1 = Z_end.x,
    Z_start_2 = Z_start.y,
    Z_end_2 = Z_end.y) %>%
  # Decide whether the Z or autosome gene is a better match for the W gene
  mutate(best_match = case_when(
    is.na(autosome_evalue) ~ "Z",
    Z_hit_1_evalue < autosome_evalue ~ "Z",
    autosome_evalue < Z_hit_1_evalue ~ "autosome",
    TRUE ~ "tie"))

# Re-arrange columns
w_z_autosomes_summary <- w_z_autosomes_summary[, c(1, 13, 14, 2, 15, 16, 3, 4, 5, 6, 17, 18, 7, 8, 9, 10, 19, 20, 11, 12, 21)]
...
w_z_autosomes_summary_v2 <- w_z_autosomes_summary %>%
  ...
  mutate(best_Z_gene = if_else(best_match == "Z", Z_hit_1, NA_character_)) %>%
  ...
  # Remove any non-reciprocal pairs when Z was considered the best match
  filter(!(reciprocal == FALSE & best_match == "Z"))
...
Z_W_gametologs <- w_z_autosomes_summary_v2 %>% 
  select(W = best_W_from_Z, W_start, W_end, Z = best_Z_gene, Z_start = Z_start_1, Z_end = Z_end_1) %>% 
  # Remove any autosome rows so that this table is just best Z-W gametolog pairs
  filter(!is.na(W)) %>% 
  # Join name columns to gametolog table
  left_join(gila_names_w, by = "W") %>% 
  left_join(gila_names_z, by = "Z") %>% 
  # Filter out any genes in the PARs of either chromosome
